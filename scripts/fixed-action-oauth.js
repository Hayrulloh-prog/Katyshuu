// Исправленная версия action-oauth маршрута
router.post('/action-oauth', async (req, res) => {
  try {
    ...
    if (lastAttendance) {
      let lastAction = 'unknown';    // Определяем последнее действие на основе checkInTime и checkOutTime
      ...
      }    // Нельзя выполнять одно и то же действие дважды подряд
      if (lastAction === action) {
        return res.status(400).json({
          error: 'Invalid action sequence',
          details: `Вы не можете выполнить "${action === 'checkin' ? 'приход' : 'уход'}" дважды подряд`
        });
      }
    ...
    let attendance;  if (action === 'checkin') {
      ...
      }    // Проверяем, не слишком ли быстро происходит checkout (менее 5 секунд после checkin)
      const checkInTime = new Date(incompleteRecord.checkInTime);
      const now = new Date();
      const timeDiff = Math.abs(now - checkInTime);    if (timeDiff < 5000) { // Менее 5 секунд
        return res.status(400).json({
